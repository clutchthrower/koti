"""Async client for the Koti REST API (Fully Kiosk Browser-compatible)."""
...
class KotiClientError(Exception):
    """Raised when a Koti API request fails."""

    def __init__(self, status_code: int, message: str) -> None:
        """Initialize the error."""
        self.status_code = status_code
        self.message = message
        super().__init__(f"KotiClientError({status_code}): {message}")


class KotiClient:
    """Client for communicating with the Koti REST API."""
    ...
    async def _send_command(self, cmd: str, **kwargs: Any) -> dict[str, Any]:
        """Send a command to the Koti API."""
        ...
        async with self._session.get(
            ...
            if response.status != 200:
                raise KotiClientError(response.status, await response.text())
            content_type = response.headers.get("Content-Type", "")
            data = await response.json(content_type=content_type)
            if isinstance(data, dict) and data.get("status") == "Error":
                raise KotiClientError(401, data.get("statustext", "Unknown error"))
            return dict(data)
